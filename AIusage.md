...
- **Výsledek:** AI (Claude Opus 4.6) mi pomohlo vymyslet a napsat logiku pro generování metrik jako `doubled_pawns`, `king_safety`, `ext_center_control` nebo `hanging_material` a integraci knihovny `python-chess` ke zjišťování ohrožených políček. Výrazně se tím zvedla přesnost.
